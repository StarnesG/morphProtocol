...
function generateSubstitutionTable() : number[] {
  const tableLength = 256;
  let substitutionTable:number[] = [];
  ...
  for (let i = tableLength - 1; i > 0; i--) {
    ...
    const temp:any = substitutionTable[i];
...
function testObfuscator() {
  ...
  const obfuscator = new Obfuscator(1234, 4, 2, {
    substitutionTable: generateSubstitutionTable(),
    randomValue: 42,
  });

  // Generate random ArrayBuffer data
  function generateRandomData(length: number): ArrayBuffer {
  ...
  function testObfuscation(data: ArrayBuffer) {
    // Obfuscate the data
